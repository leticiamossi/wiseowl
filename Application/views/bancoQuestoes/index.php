...
<body onload="filtrar()">
    ...
    <script>
        function filtrar() {
            ...
            if (assunto != "Todos") {
                json = json.filter(q => q.assunto_assunto == assunto)
            }
            if (topico != "") {
                json = json.filter(q => q.id_assunto == topico)
            }
            if (tamanho != "Todos") {
                var min = 0
                var max = 0
                ...
                json = json.filter(q => q.tamanho_questao >= min && q.tamanho_questao <= max)
            }
