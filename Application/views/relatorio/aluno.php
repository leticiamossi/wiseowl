...
<body onload="carregar()">
    ...
    <main class="fundo-roxo">
        <div class="caixa">
            <h3 class="titulo-main">Relatório do Aluno</h3>
            ...
            <div class="filtro-linha" style="margin-bottom: 50px;">
                ...
                <script>
                    function filtrar() {
                        ...
                        var jsonT = JSON.parse('<?= json_encode($data['turma']); ?>');
                        ...
                        if (data != '') {
                            ...
                            jsonT = jsonT.filter(p => p.data_prova.substr(0, 7) == data)
                        }

                        if (materia !== "") {
                            json = json.filter(j => j.nome_materia == materia)
                            jsonT = jsonT.filter(j => j.nome_materia == materia)
                        }
                        if (assunto !== "") {
                            json = json.filter(j => j.assunto_assunto == assunto)
                            jsonT = jsonT.filter(j => j.assunto_assunto == assunto)
                        }
                        if (topico !== "") {
                            json = json.filter(j => j.topico_assunto == topico)
                            jsonT = jsonT.filter(j => j.topico_assunto == topico)
                        }

                        atualizarRelatorioAluno(json, jsonT);
